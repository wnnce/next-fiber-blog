...
func (self *ConcatRepo) Save(concat *usercase.Concat) error {
	builder := sqlbuild.NewInsertBuilder("t_blog_concat").
		Fields("name", "icon_svg", "target_url", "is_main", "sort", "status").
		Values(concat.Name, concat.IconSvg, concat.TargetUrl, concat.IsMain, concat.Sort, concat.Status).
		Returning("concat_id")
...
func (self *ConcatRepo) List() ([]*usercase.Concat, error) {
	builder := sqlbuild.NewSelectBuilder("t_blog_concat").
		Select("concat_id", "name", "icon_svg", "target_url", "is_main").
